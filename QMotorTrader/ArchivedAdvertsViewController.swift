//
//  ArchivedAdvertsViewController.swift
//  Minao
//
//  Created by Mahmood Nassar - (+970598197338) on 9/5/15.
//  Copyright (c) 2015 Minao. All rights reserved.
//

import UIKit

class ArchivedAdvertsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var archivedCarsLabel: UILabel!
...
//        let name = "ArchivedAdvertsViewController"
...
//        tracker.send(builder.build() as [NSObject : AnyObject])
    ...
    override func viewDidLoad() {
        ...
        archivedCarsLabel.text = utility.__("archivedCarsLabel")
        controlItemLabel.text = utility.__("controlItemLabel")
        
        worker.setUp(context: managedObjectContext!, domain: "qmotor.com")
        refreshControl.tintColor = UIColor.lightGrayColor()
        refreshControl.addTarget(self, action: #selector(ArchivedAdvertsViewController.getOnlineCars as (ArchivedAdvertsViewController) -> () -> ()), forControlEvents: UIControlEvents.ValueChanged)
    ...
    func getOnlineCars(haveRefreshControl: Bool) {
        ...
        let fullUrl = "adverts?act=adverts&ch=archive&page=\(self.nextCursor)&rowCount=\(self.limit)"
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            ...
            cell.advertExpiredAt.text = utility.__("advertExpiredAt")
            return cell
        } else {
        ...
        if hasMore && isReady {
            if offsetY > contentHeight - scrollView.frame.size.height {
                ...
                if utility.isConnectedToNetwork() {
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0)) {
                        self.worker.makeUrl("adverts?act=adverts&ch=archive&&page=\(self.nextCursor)&rowCount=\(self.limit)").requestAndSave(Worker.Method.GET, parameters: [:]) {
                            (resultset, pagination) in
                            if let carsList = resultset?.dictionaryValue[self.controller] {
                                for (_, car) in carsList {
                                    var record = [String: String]()
